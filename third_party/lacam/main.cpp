...
int main(int argc, char* argv[])
{
  ...
  // Construct instance on the heap to avoid copy/move issues
  // (Graph's destructor deletes vertices, but has no move constructor)
  std::unique_ptr<Instance> ins_ptr;
  if (scen_name.size() > 0) {
    ins_ptr = std::make_unique<Instance>(scen_name, map_name, N);
  } else {
    ins_ptr = std::make_unique<Instance>(map_name, &MT, N);
  }
  const auto& ins = *ins_ptr;
