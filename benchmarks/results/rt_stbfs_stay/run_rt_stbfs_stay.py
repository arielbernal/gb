#!/usr/bin/env python3
"""Run het_rt_lacam RT+ST-BFS+stay benchmark for a single category."""
import argparse
import csv
import re
import subprocess
...
OUT_DIR = ROOT / "benchmarks" / "results" / "rt_stbfs_stay"
...
CSV_HEADER = [
    ...
    "runtime_ms", "rt_steps", "move_steps", "stay_steps", "explored",
]
...
def run_one(scen, timeout_s, rt_budget):
    ...
    move_steps = stay_steps = explored = 0
    m = re.search(r"move=(\d+)\s+stay=(\d+)\s+explored=(\d+)", output)
    if m:
        move_steps = int(m.group(1))
        stay_steps = int(m.group(2))
        explored = int(m.group(3))
    ...
    return {
        "solver": "het_rt_lacam_rt_stbfs_stay",
...
def main():
    ...
    for i, scen in enumerate(scenarios):
        ...
        if r["solved"]:
            detail = f"steps={r['rt_steps']} soc={r['soc']} mk={r['makespan']} move={r['move_steps']} stay={r['stay_steps']} t={r['runtime_ms']}ms"
        else:
            detail = f"steps={r['rt_steps']} move={r['move_steps']} stay={r['stay_steps']} expl={r['explored']} t={r['runtime_ms']}ms"
