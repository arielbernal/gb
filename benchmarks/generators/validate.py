...
Checks:
  1. All start positions valid (NxN footprint all free, within bounds)
  2. All goal positions valid (same)
  3. No two agents' start footprints overlap
  4. No two agents' goal footprints overlap
  5. Each agent's goal is BFS-reachable from start (treating agent as NxN block)
...
def validate(map_path, scen_path):
    ...
    # Check 3: No start overlap
    check3_ok = True
    print("\n=== Check 3: No start footprint overlaps ===")
    ...
    # Check 4: No goal overlap
    check4_ok = True
    print("\n=== Check 4: No goal footprint overlaps ===")
    for i in range(len(agents)):
    ...
    # Check 5: BFS reachability
    print("\n=== Check 5: BFS reachability (NxN block) ===")
    check5_ok = True
    for i, (cs, sx, sy, gx, gy) in enumerate(agents):
